...
LeptonThread::LeptonThread() : QThread()
{
	...
	//
	typeColormap = 2; // 1:colormap_rainbow  /  2:colormap_grayscale  /  3:colormap_ironblack(default)
	...
	while(true) {
		...
		for(int iSegment = iSegmentStart; iSegment <= iSegmentStop; iSegment++) {
			...
			for(int i=0;i<FRAME_SIZE_UINT16;i++) {
				...
				temptemp = valueFrameBuffer * 0.007 - 183;
				uint8_t LSB = (valueFrameBuffer & 0x00FF);
				uint8_t MSB = ((valueFrameBuffer & 0xFF00) >>8);
				color = qRgb(MSB,LSB,0);
		...
		count+=1;
		if (count>10){count=1;}
		QString nomImg = QString(tr("%1.png")).arg(count);
		myImage.save(nomImg);
		
		//myImage.save("imag.png");
